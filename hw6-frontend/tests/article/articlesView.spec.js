...
describe('ArticlesView', () => {
	...
	it('should render articles', (done) => {
		const node = TestUtils.renderIntoDocument((<div>
            <Articles username='Tony' articles={ articles }
                EditArticle={ () => <div /> }
            	Comments={ () => <div /> } onEdit={ () => _ }/>
        </div>))
        ...
        expect(article.children.length).to.eql(4)
	...
	it('should dispatch actions to create a new article', (done) => {
        ...
        const cancel = form.children[1].children[2].children[0]
    ...
    it('should dispatch actions to create a new article', (done) => {
        let file
        const newArticle = (_, image) => file = image
        const newText = 'new article'
        const node = TestUtils.renderIntoDocument((<div>
            <NewArticle onPostArticle={ newArticle }/>
        </div>))
        const form = node.children[0]
        const fileInput = form.children[1].children[1].children[0].children[1]
        TestUtils.Simulate.change(fileInput, { target: 
            { files: [new File([], 'unused')] } })
        const btn = form.children[1].children[0].children[0]
        TestUtils.Simulate.click(btn)
