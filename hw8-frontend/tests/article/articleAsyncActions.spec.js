...
describe('Validate Article asyncronous actions', () => {
        ...
        store.dispatch(actions.setAllArticles())
            .then(() => {
                expect(store.getState().type).to
                    .eql(actions.Action.UPDATE_ARTICLES_EXCLUSIVE)
    ...
    it('should post new article', (done) => {
        ...
        store.dispatch(actions.postArticle('unused'))
            .then(() => {
                expect(store.getState().type).to
                    .eql(actions.Action.UPDATE_ARTICLES_INCLUSIVE)
    ...
    it('should post new article with images', (done) => {
        mock(`${url}/article`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            json: {
        ...
        store.dispatch(actions.postArticle('unused', new File([], 'unused')))
            .then(() => {
                expect(store.getState().type).to
                    .eql(actions.Action.UPDATE_ARTICLES_INCLUSIVE)
                expect(store.getState().articles).to.eql(articlesByAuthor)
